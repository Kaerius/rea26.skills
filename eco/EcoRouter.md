...
> MTU на VPLS ведущий в интернет 1500, остальные MTU оставляем по умолчанию. MTU на клиентах ставим 1440

> Сеть 192.168.100.0/24 предназначена для доступа по SSH и SNMP, вход через VLAN 100 от CR-SRV, он будет и шлюзом для остальных в эту сеть. Нужно будет прописать маршруты руками на CR-SRV. И
...
> **Важно:**  
> - На `MPLS-GW-BR` отсутствует сервис-инстанс `SNMP_SSH` — управление осуществляется через MPLS-GW-CR по loopback интерфейсам.
