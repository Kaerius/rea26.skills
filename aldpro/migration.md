...
2. Перейдите в раздел `Роли и службы` → `Служба разрешения имён` → `Перенаправление запросов` 
3. Добавьте новый форвардер:
   - **Имя зоны:** `tech.skills`
   - **Глобальные перенаправители:** `192.168.1.15`
   - **Политики перенаправления:** `Только перенаправлять`
   - **Состояние зоны DNS:** `Включено`
4. Сохраните настройки

---

## Шаг 4: Проверка видимости AD через ldapsearch
Выполните на сервере ALD (нужно доработь):
```bash
